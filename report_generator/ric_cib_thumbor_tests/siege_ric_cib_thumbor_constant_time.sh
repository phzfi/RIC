...
CIB_OUT_FILE=./results/cib_CT_"$SEED"_"$CONCURRENT"_"$TIME"_$(date +%Y-%m-%d_%H-%M-%S).csv
TMP=./temp/$(date +%Y-%m-%d_%H-%M-%S).tmp
...
THUMBOR_OUT_FILE=./results/thumbor_CT_"$SEED"_"$CONCURRENT"_"$TIME"_$(date +%Y-%m-%d_%H-%M-%S).csv
TMP=./temp/$(date +%Y-%m-%d_%H-%M-%S).tmp
...
python3 csv_to_html.py html_tables/ricCibThumborConstantTimeResults.html $RIC_OUT_FILE $CIB_OUT_FILE $THUMBOR_OUT_FILE
