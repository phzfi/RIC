...
THUMBOR_OUT_FILE_BEFORE=./results/thumbor-before_CTLA_"$SEED"_"$CONCURRENT"_"$TIME"_$(date +%Y-%m-%d_%H-%M-%S).csv
THUMBOR_OUT_FILE_AFTER=./results/thumbor-after_CTLA_"$SEED"_"$CONCURRENT"_"$TIME"_$(date +%Y-%m-%d_%H-%M-%S).csv
TMP=./temp/$(date +%Y-%m-%d_%H-%M-%S).tmp
...
python3 csv_to_html.py html_tables/ricCibThumborConstantTimeResultsLocalAuto.html $RIC_OUT_FILE_BEFORE $RIC_OUT_FILE_AFTER $CIB_OUT_FILE_BEFORE $CIB_OUT_FILE_AFTER $THUMBOR_OUT_FILE_BEFORE $THUMBOR_OUT_FILE_AFTER
